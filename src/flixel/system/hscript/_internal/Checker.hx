...
import flixel.system.hscript._internal.Parser;
...
@:allow(flixel.system.hscript._internal.Checker)
class CheckerTypes {
    ...
    function addXmlType(x:haxe.rtti.CType.TypeTree, todo:Array<Void->Void>) {
        switch (x) {
                ...
                todo.push(function() {
                    ...
                    function initField(f:haxe.rtti.CType.ClassField, fields) {
                        ...
                        if (f.meta != null && f.meta.length > 0) {
                            fl.meta = [];
                            for (m in f.meta)
                                fl.meta.push({name: m.name, params: [
                                    for (p in m.params) try parser.parseString(p) catch (e:flixel.system.hscript._internal.Expr.Error) null
                                ]});
                        }
            ...
            case TTypedecl(t):
                ...
                if (t.path == "flixel.system.hscript._internal.TypeCheck")
                    td.params.reverse();
                todo.push(function() {
    ...
    function addMeta(src:haxe.rtti.CType.TypeInfos, to:CNamedType) {
        ...
        to.meta = [];
        for (m in src.meta)
            to.meta.push({name: m.name, params: [
                for (p in m.params) try parser.parseString(p) catch (e:flixel.system.hscript._internal.Expr.Error) null
            ]});
    }
...
class Checker {
    ...
    public function check(expr:Expr, ?withType:WithType, ?isCompletion = false) {
        if (withType == null)
            withType = NoValue;
        locals = new Map();
        @:privateAccess if (types.t_string == null)
    ...
    public static function typeStr(t:TType) {
        ...
        return switch (t) {
            ...
            case TType(t, args):
                if (t.name == "flixel.system.hscript._internal.TypeCheck") typeStr(args[1]); else t.name + makeArgs(args);
