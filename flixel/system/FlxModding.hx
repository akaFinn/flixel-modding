...
class FlxModding
{
	 ...
	 */
	static var hScriptExt(get, null):String;

	static function get_hScriptExt():String
	{
		if (hScriptExt == null)
			hScriptExt = flixel.util.FlxModUtil.getDefinedString("FM_HSCRIPT_EXT", ".hxs");
		return hScriptExt;
	}
	 ...
	 */
	static var polymodScriptExt(get, null):String;

	static function get_polymodScriptExt():String
	{
		if (polymodScriptExt == null)
			polymodScriptExt = flixel.util.FlxModUtil.getDefinedString("POLYMOD_SCRIPT_EXT", ".hxc");
		return polymodScriptExt;
	}

	/**
	 * File extension used for RuleScript classes.
	 */
	static var ruleScriptExt(get, null):String;

	static function get_ruleScriptExt():String
	{
		if (ruleScriptExt == null)
			ruleScriptExt = flixel.util.FlxModUtil.getDefinedString("FM_RULESCRIPT_EXT", '.rhx');
		return ruleScriptExt;
	}

	public static var scripting(get, null):Null<Bool>;

	static function get_scripting():Bool
	{
		if (scripting == null)
			scripting = flixel.util.FlxModUtil.getDefinedBool("FM_SCRIPTING", true);
		return scripting;
	}
			...
			?assetDirectory:String, ?modsDirectory:String):FlxModding
	{
		...
		flixel.system.FlxModding.assetDirectory = assetDirectory ?? flixel.system.FlxModding.assetDirectory;
		flixel.system.FlxModding.modsDirectory = modsDirectory ?? flixel.system.FlxModding.modsDirectory;
		...
		{
			flixel.system.FlxModding.customModpack = customModpack ?? flixel.system.FlxModding.customModpack;
			flixel.system.FlxModding.customFormat = customFormat ?? flixel.system.FlxModding.customFormat;
	 ...
	 */
	public static function create(fileName:String, iconBitmap:BitmapData, metadata:FlxBaseMetadataFormat, ?type:FlxModpackType = FLIXEL,
